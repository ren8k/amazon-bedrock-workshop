...
class LLM():
    def __init__(self, region, is_stream=False) -> None:
        retry_config = Config(
            region_name=region,
            retries={
                "max_attempts": 10,
        ...
        self.bedrock_runtime = boto3.client('bedrock-runtime',
                                            config=retry_config, region_name=region)
        if is_stream:
    ...
    def _get_generated_text(self, response_body, model_id):
    ...
    def _get_generated_stream_text(self, event, model_id):
        if "claude-3" in model_id:
            ...
            if chunk['event_type'] == 'text-generation':
                ...
                print(f"Output tokens: {chunk['amazon-bedrock-invocationMetrics']['outputTokenCount']}")
